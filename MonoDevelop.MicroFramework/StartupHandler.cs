	...
	class StartupHandler : CommandHandler
	{
		...
		protected override void Run()
		{
			if(Platform.IsMac)
			{
				...
				var registryKey = Registry.CurrentUser.OpenSubKey("Software\\Microsoft\\.NETMicroFramework\\v4.2", true);
				if(registryKey == null)
				{
					registryKey = Registry.CurrentUser.CreateSubKey("Software\\Microsoft\\.NETMicroFramework\\v4.2");
				}
				if(registryKey.GetValue("InstallRoot") == null)
				{
					registryKey.SetValue("BuildNumber", "1");
					registryKey.SetValue("RevisionNumber", "0");
					registryKey.SetValue("InstallRoot", "/Library/Frameworks/Microsoft .NET Micro Framework/v4.2");
				}
				...
				if(!Directory.Exists("/Library/Frameworks/Mono.framework/External/xbuild-frameworks/.NETMicroFramework") ||
				   !File.Exists("/Library/Frameworks/Mono.framework/External/xbuild-frameworks/.NETMicroFramework/v4.2/Microsoft.SPOT.Hardware.PWM.dll"))
				{
				...
				if(!Directory.Exists("/Library/Frameworks/Mono.framework/Versions/Current/lib/mono/xbuild/Microsoft/.NET Micro Framework") ||
				   (GetChecksum("/Library/Frameworks/Mono.framework/Versions/Current/lib/mono/xbuild/Microsoft/.NET Micro Framework/v4.2/CSharp.targets") != GetChecksum(Path.Combine(addInFolder, "files/xbuild/Microsoft/.NET Micro Framework/v4.2/CSharp.targets"))))
				{
				...
				if(!Directory.Exists("/Library/Frameworks/Microsoft .NET Micro Framework/v4.2/") ||
				   (GetChecksum("/Library/Frameworks/Microsoft .NET Micro Framework/v4.2/Tools/MetaDataProcessor.exe") != GetChecksum(Path.Combine(addInFolder, "files/frameworks/Microsoft .NET Micro Framework/v4.2/Tools/MetaDataProcessor.exe"))) ||
				   !File.Exists("/Library/Frameworks/Microsoft .NET Micro Framework/v4.2/Tools/MetaDataProcessor"))
				{
					DirectoryCopy(Path.Combine(addInFolder, "files", "frameworks/"),
						"/Library/Frameworks/");
					RunAppleScript("do shell script \"chmod +x \\\"/Library/Frameworks/Microsoft .NET Micro Framework/v4.2/Tools/MetaDataProcessor.exe\\\"\" with administrator privileges");
					RunAppleScript("do shell script \"chmod +x \\\"/Library/Frameworks/Microsoft .NET Micro Framework/v4.2/Tools/MetaDataProcessor\\\"\" with administrator privileges");
					newlyInstalled = true;
				}
				if(newlyInstalled)
				{
